<template>
  <my-dialog title="Xác nhận" width="500" 
    ...
    <template #content>
      <p>Bạn có chắc chắn xóa danh mục chi tiết <br/><span class="text-warning font-weight-bold">{{ props.data?.itemname || `Không rõ danh mục chi tiết` }}</span></p>
    </template>
    <template #action>
...
import { updateStatusItem } from '@/apis/modules/items.api';
...
async function onDelete() {
  try {
    let res = await updateStatusItem({
      "_id": props.data?._id, 
