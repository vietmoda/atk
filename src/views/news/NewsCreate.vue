<template>
  <my-dialog 
    v-model="props.isShow" title="Tạo mới bài viết" width="90%" v-on:dismiss="emit('update:isShow', false)">
    <!-- child template -->
    <template #content >
...
<script setup>
  ...
  import { createNews } from '@/apis/modules/news.api';
  ...
  async function save() {
    ...
    if(isValid.value) {
      try {
        ...
        body.publishdate = body.newsstatus == NEWS_STATUS.CREATED ? null : moment(body.publishdate, 'YYYY-MM-DD').format('DD/MM/YYYY')
        console.log(body)
        console.log(data.selectNewCategoryId)

        //save
        let res = await createNews(body);
        if (res.error == "0"){
