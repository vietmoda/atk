<template>
  <my-dialog title="Xác nhận" width="500" 
    ...
    <template #content>
      <p>Bạn có chắc chắn xóa khách hàng <br/><span class="text-warning font-weight-bold">{{ props.data?.visitername || `Không rõ khách hàng` }}</span></p>
    </template>
    <template #action>
...
import { updateStatusBooking } from '@/apis/modules/bookings.api';
...
async function onDelete() {
  try {
    let res = await updateStatusBooking({
      "_id": props.data?._id, 
