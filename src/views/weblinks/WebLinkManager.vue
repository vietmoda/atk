<template>
  <v-card class="mb-5 card-search">
    <v-toolbar class="title" color="primary" title="Danh sách kết nối">
    </v-toolbar>
    <v-card-text>
      <v-row class="mt-2 mb-1">
        <v-col cols="12" md="2"></v-col>
        <v-col cols="12" md="5">
          <v-select v-model="data.selectNewCategoryId" @update:modelValue="onSearch" variant="outlined"
            label="Chuyên mục liên kết" :items="newCategories" item-title="label" item-value="value" class="ml-lg-3"
            return-object />
        </v-col>
        <v-col cols="12" md="3">
    ...
    <v-card-actions>
      <v-row>
        <v-col cols="12" md="12" class="d-flex justify-center align-baseline">
          ...
          <v-btn variant="outlined" color="success" @click="createDialogState = true">
  ...
  <v-card class="mx-auto mt-5">
    <vue-good-table ref="someTable" :columns="columns" :rows="webLinks" :line-numbers="true" :loading="loading"
      :sort-options="{
        enabled: true
      }" :search-options="{
  enabled: true,
  ...
  ofLabel: 'của',
      ...
      <template #table-row="props">
        ...
        <div v-else-if="props.column.field === 'action'" class="d-flex justify-center">
          <v-btn @click="updateWebLink(props.row)" v-bind="props" variant="outlined" size="x-small" color="secondary"
            class="mx-1" icon="mdi-pencil">
            <v-icon color="secondary">mdi-pencil</v-icon>
            <v-tooltip activator="parent" location="top">Sửa liên kết</v-tooltip>
          </v-btn>
          <v-btn @click="deleteWebLink(props.row)" variant="outlined" size="x-small" color="error" class="mx-1"
            icon="mdi-delete">
            <v-icon color="error">mdi-delete</v-icon>
            <v-tooltip activator="parent" location="top">Xóa liên kết</v-tooltip>
          </v-btn>
        </div>
      </template>
    </vue-good-table>
  ...
  <WebLinkCreate v-model:is-show="createDialogState" v-on:done="onSearch" />
  <WebLinkUpdate v-model:is-show="updateDialogState" :data="dataUpdate" v-on:done="onSearch" />
  <WebLinkDelete v-model:is-show="deleteDialogState" :data="dataDelete" v-on:done="onSearch" />
</template>

<script setup>
import { reactive, ref, onMounted } from 'vue'
import WebLinkCreate from './WebLinkCreate.vue';
import WebLinkUpdate from './WebLinkUpdate.vue'
import WebLinkDelete from './WebLinkDelete.vue'
import { getWebLinkByCategoryId } from '@/apis/modules/weblinks.api';
...
async function onSearch() {
  let res = await getWebLinkByCategoryId(data.selectNewCategoryId.value, data.selectStatus.value);
  if (res && res.data)
    webLinks.value = res.data
}
</script>
