<template>
  <my-dialog title="Xác nhận" width="500" 
    ...
    <template #content>
      <p>Bạn có chắc chắn xóa liên kết <br/><span class="text-warning font-weight-bold">{{ props.data?.linkname || `Không rõ liên kết` }}</span></p>
    </template>
    <template #action>
...
import { updateStatusWebLink } from '@/apis/modules/weblinks.api';
...
async function onDelete() {
  try {
    let res = await updateStatusWebLink({
      "_id": props.data?._id, 
