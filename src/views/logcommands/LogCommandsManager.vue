<template>
  <v-card class="mb-5 card-search">
    <v-toolbar class="title mb-3" color="primary" title="Lịch sử phiên bản"></v-toolbar>
    <v-card-text>
      <v-row class="mt-2 mb-1">
        <v-col cols="12" md="4">
          <v-select v-model="data.actiontype" @update:modelValue="onSearch(1)" variant="outlined" label="Thao tác"
            :items="actionType" item-title="label" item-value="value" class="ml-lg-3" return-object />
          <!-- <v-text-field v-model="data.actiontype" v-on:keyup.enter="onSearch(1)" label="Tên thao tac"
                        placeholder="Nhập thao tac truy xuat" variant="outlined" type="input" class="ml-lg-3" /> -->
        </v-col>
        <v-col cols="12" md="4">
          <v-select v-model="data.collectionname" @update:modelValue="onSearch(1)" variant="outlined"
            label="Bảng truy xuất" :items="collectionName" item-title="label" item-value="value" class="ml-lg-3"
            return-object />
          <!-- <v-text-field v-model="data.collectionname" v-on:keyup.enter="onSearch(1)" label="Tên bang thao tac"
                        placeholder="Nhập tên bang thao tac truy xuat" variant="outlined" type="input" class="ml-lg-3" /> -->
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field v-model="data.keyword" v-on:keyup.enter="onSearch(1)" label="keyword"
            placeholder="Nhập keyword tìm kiếm" variant="outlined" type="input" class="ml-lg-3" />
        </v-col>
        <v-col cols="12" md="2"></v-col>
        <v-col cols="12" md="4">
          <v-text-field v-model="data.startdate" v-on:keyup.enter="onSearch(1)" label="Ngày bắt đầu"
            placeholder="dd/mm/yyyy" variant="outlined" type="date" class="ml-lg-3" />
        </v-col>
        <v-col cols="12" md="4">
          <v-text-field v-model="data.enddate" v-on:keyup.enter="onSearch(1)" label="Ngày kết thúc"
            placeholder="dd/mm/yyyy" variant="outlined" type="date" class="ml-lg-3" />
        </v-col>
        <v-col cols="12" md="2"></v-col>
    ...
    <v-card-actions>
      <v-row>
        <v-col cols="12" md="12" class="d-flex justify-center align-baseline">
          <v-btn variant="outlined" color="orange-darken-2" prepend-icon="mdi-magnify" @click="onSearch(1)">
            Tìm kiếm
          </v-btn>
        </v-col>
      </v-row>
