...
const { updateUserAction } = useUserStore();
  ...
  Object.assign(body, 
    {
      ...
      organizationid: organizations.find(o=> o.value == Number(props.data?.organizationid))
    });
  
  const [avatar] = props.data.avatar || [];
  avatarSrc.value = avatar ? `${MEDIA_SERVER}/${avatar?.fileurl}` : undefined;
});

async function save() {
  console.log(body)
  await updateUserAction(body);
  // dismiss Dialog when save done.
  dismissDialog();
  emit('done');
}
