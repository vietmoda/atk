<template>
  <my-dialog title="Xác nhận" width="500" 
    ...
    <template #content>
      <p>Bạn có chắc chắn xóa khách hàng <br/><span class="text-warning font-weight-bold">{{ props.data?.fullname || `Không rõ khách hàng` }}</span></p>
    </template>
    <template #action>
...
import { updateStatusOrderMenu } from '@/apis/modules/ordermenus.api';
...
async function onDelete() {
  try {
    let res = await updateStatusOrderMenu({
      "_id": props.data?._id, 
