<template>
  <my-dialog 
    v-model="props.isShow" title="Đặt bàn" width="1200" v-on:dismiss="emit('update:isShow', false)">
    <!-- child template -->
    <template #content>
...
<script setup>
  ...
  import { createOrderMenu } from '@/apis/modules/ordermenus.api';
  ...
  async function save(){
    ...
    if(isValid.value) {
      try {
        ...
        //save
        let res = await createOrderMenu(body);
        if (res.error == "0"){
