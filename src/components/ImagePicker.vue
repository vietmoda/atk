...
const emit = defineEmits(['image-picker-upload-done', 'upload:done']);
...
async function uploadFiles() {
  ...
  vueFileAgent.value.upload(data.uploadUrl, data.uploadHeaders, data.fileRecordsForUpload, function createFormData(fileData){
    ...
    response.map(item => {
      ...
      fileUploads.value.push(data[0]);
      showButtonUploadDone.value = true;
      //emit('image-picker-upload-done', data)
      //return data;
    })
  }).catch(e => {
    console.log(e)
...
function uploadFileDone() {
  ...
  emit('image-picker-upload-done', fileUploads.value)
  fileUploads.value=[];
}
