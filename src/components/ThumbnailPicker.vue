...
<template>
<my-dialog 
  v-model="props.isShow" title="Tải ảnh" width="600" v-on:dismiss="emit('update:isShow', false)">
  <template #content>
  ...
  <template #action>
    <v-btn class="mt-3" variant="outlined" color="success" @click="uploadFiles()">Bắt đầu tải</v-btn>
    <v-btn class="mt-3 ml-1" variant="outlined" color="primary" :disabled="!showButtonUploadDone" @click="uploadFileDone">Hoàn thành</v-btn>
  </template>
</my-dialog>
</template>
<script setup>
...
const emit = defineEmits(['thumbnail-picker-upload-done', 'upload:done']);

const fileUploads = ref(null)
const uploadFileDialogState = ref(false);
...
async function uploadFiles() {
  ...
  vueFileAgent.value.upload(data.uploadUrl, data.uploadHeaders, data.fileRecordsForUpload, function createFormData(fileData){
    ...
    data.filesUploaded = response.map(item => {
      ...
      fileUploads.value = data;
      showButtonUploadDone.value = true;
      //emit('thumbnail-picker-upload-done', data)
      //return data;
    })
  }).catch(e => {
    console.log(e)
...
function uploadFileDone() {
    ...
    emit('thumbnail-picker-upload-done', fileUploads.value)
}
